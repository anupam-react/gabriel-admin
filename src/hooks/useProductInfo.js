...
const useProductInfo = () => {
  ...
  const getCategorybyId = async (id)=>{
    const data = await fetchApiData(`https://gabriel-backend.vercel.app/api/v1/admin/Category/getCategory/${id}`)
    setCategotyInfo(data?.data)
  }
  const getSubcategoryById = async (id)=>{
    const data = await fetchApiData(`https://gabriel-backend.vercel.app/api/v1/SubCategory/${id}`)
